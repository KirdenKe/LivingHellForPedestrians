        ...
        public async Task LoadAccident()
        {
            ...
            */
            ...
            for(int i = 3; i < MaxIndex; i += 13)
            {
                ...
                //把資料丟入List裡
                LivingHell.Add(new TPH(year, int.Parse(document.QuerySelector(AccidentTextSelector).InnerHtml.Replace(",", "")), int.Parse(document.QuerySelector(DeathTextSelector).InnerHtml.Replace(",", "")), int.Parse(document.QuerySelector(InjuredTextSelector).InnerHtml.Replace(",", ""))));
                /*其他縣市
            ...
            for (int i = 3; i < MaxIndex; i += 13)
            {
                ...
                {
                    if (LivingHell[j].GetType() == typeof(TPH)) 
                    {
                        var livingHell = (TPH)LivingHell[j];
                        if(livingHell.Year == year)
                        {
